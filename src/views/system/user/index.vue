<template>
  <div class="table-container">
    ...
    >
      ...
      <el-table-column prop="company_user_id" label="ID" width="80"/>
...
import {getUserList, saveUser, updateUser, changeUserStatus, deleteUser} from '@/api/system/user'
...
const handleDelete = async (row) => {
  ...
  await handleApiCall(
    () => deleteUser(row.company_user_id),
    '删除成功'
  )
  getTableData()
}
...
const handleBatchDelete = async () => {
  const ids = selectedRows.value.map(row => row.company_user_id)
...
const handleSubmit = async () => {
  ...
  const apiCall = formData.value.company_user_id
    ? () => updateUser(formData.value.company_user_id, formData.value)
